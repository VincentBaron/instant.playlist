...
// The reposts stream mixes track- and playlist-reposts; each wraps the reposted item.
interface SCRepostItem {
  type?: string; // "track-repost" | "playlist-repost"
  track?: SCRawTrack;
  playlist?: SCRawTrack; // playlists share the fields we read (title/url/duration)
}

/** The reposted track or playlist inside a stream item, whichever is present. */
function repostTrack(item: SCRepostItem): SCRawTrack | null {
  return item.track ?? item.playlist ?? null;
}

/**
 * Newest reposted item whose duration ≥ threshold — a DJ *set* the artist has reposted.
 * Many DJs upload nothing themselves and instead repost the sets they play (this is the
 * YOL-37 case: an account with 0 own tracks looks "not found" but has reposted sets).
 * The stream is newest-first, so we page and take the first item over the line.
 */
export async function getLatestRepostSet(
  userId: number,
  minDurationMs: number,
  maxPages = 2,
): Promise<Track | null> {
  let url: string | undefined =
    `${BASE}/stream/users/${userId}/reposts?limit=50&linked_partitioning=1`;
  for (let page = 0; page < maxPages && url; page++) {
    const data = (await scGet(url)) as SCPage<SCRepostItem>;
    for (const item of data.collection ?? []) {
      const raw = repostTrack(item);
      if (!raw) continue;
      const ms = raw.full_duration ?? raw.duration ?? 0;
      if (ms >= minDurationMs) return toTrack(raw);
    }
...
/** Last-resort fallback: the artist's most recent reposted tracks (own uploads absent). */
export async function getTopReposts(
  userId: number,
  limit = 3,
): Promise<Track[]> {
  const data = (await scGet(
    `${BASE}/stream/users/${userId}/reposts?limit=50&linked_partitioning=1`,
  )) as SCPage<SCRepostItem>;
  const tracks: Track[] = [];
  for (const item of data.collection ?? []) {
    const raw = repostTrack(item);
    if (raw && raw.permalink_url) tracks.push(toTrack(raw));
